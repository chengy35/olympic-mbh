function trainAndTest_normalizedL2_FV_LLC(video_data_dir,fullvideoname,featDir_FV,featDir_LLC,encode,actionName)
	...
	fvTrainData = load('fvTrainData');
	fvTestData = load('fvTestData');
	if ~exist('TestData_Kern_cell.mat','file')
			...
			for j = 1:size(trn_indx,1)
				...
				llctemp =  dlmread(featFile{j});
				llctemp = llctemp';
				temp = [normalizeL2(sqrt(abs(llctemp'))') fvTrainData.TrainData(j,:)];
				TrainData(j,:)  = temp';
				clear temp;
				clear llctemp;
			end
			...
			for j = 1:size(test_indx,1)
				...
				llctemp =  dlmread(featFile{j});
				llctemp = llctemp';
				temp = [normalizeL2(sqrt(abs(llctemp'))') fvTestData.TestData(j,:)];
				TestData(j,:)  = temp';
				clear temp;
				clear llctemp;
			end
